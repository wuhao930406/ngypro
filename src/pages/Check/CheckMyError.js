...
@connect(({ check, global, loading }) => ({
  check, global,
  submitting: loading.effects['check/myErrorqueryMyList'],
}))
class CheckMyError extends React.Component {
  ...
  render() {
    let { postData, postUrl, fv, fields, iftype, curitem } = this.state,
      { myErrorqueryMyList, rslgetRepairDetail, dataList } = this.props.check;
    const columnes = [
      {
        title: '备件料号',
    ...
    let pageChange = (page) => {
      this.setState({
        ...
        this.setNewState("myErrorqueryMyList", this.state.postData);
      })
    }
    ...
    return (
      <div>
        ...
        <Card title='我的点检异常' extra={curitem.id && curitem.status == "1" ?
          ...
          <Table size="middle"
            ...
            pagination={{
              ...
              current: myErrorqueryMyList.pageNum ? myErrorqueryMyList.pageNum : 1,
              total: myErrorqueryMyList.total ? parseInt(myErrorqueryMyList.total) : 1,
              onChange: pageChange,
            }}
            rowKey='id'
            columns={columns}
            dataSource={myErrorqueryMyList.list ? myErrorqueryMyList.list : []}
          >
          </Table>

...
@connect(({ check, loading }) => ({
  check,
  submitting: loading.effects['check/equipmentqueryLists'],
...
class checkSetting extends React.Component {
  ...
  render() {
    let { postData, postUrl, fv, fields, iftype, curitem, charge, selectedRowKeys, selectedRowKeyc, postDatas, postDatac } = this.state,
      { equipmentqueryLists, userList, equipmentqueryNoByUserId, equipmentqueryByUserId } = this.props.check;
    const rowSelections = {
    ...
    const columns = [
      ...
      //   dataIndex: 'action',
      //   key: 'action',
      //   render:(text,record)=><Link to={`/check/checksetting/checkhistory/${record.equipmentId}/${record.equipmentName}`}>查看点检历史</Link>
      // },
    ]
    ...
    let pageChange = (page) => {
      this.setState({
        ...
        this.setNewState("equipmentqueryLists", this.state.postData);
      })
    }
          ...
          <div>
            <Table size="middle"
              ...
              pagination={{
                ...
                current: equipmentqueryLists.pageNum ? equipmentqueryLists.pageNum : 1,
                total: equipmentqueryLists.total ? parseInt(equipmentqueryLists.total) : 1,
                onChange: pageChange,
              }}
              rowKey='id'
              columns={columns}
              dataSource={equipmentqueryLists.list ? equipmentqueryLists.list : []}
            >
            </Table>
          </div>
          <Modal

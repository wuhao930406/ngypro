...
class checkSetting extends React.Component {
  ...
  render() {
    let { postData, postUrl, fv, fields, iftype, curitem, charge, selectedRowKeys, selectedRowKeyc, postDatas, postDatac } = this.state,
      { equipmentqueryList, userList, equipmentqueryNoByUserId, equipmentqueryByUserId, deviceTypequeryTreeList } = this.props.check;
    const rowSelections = {
        ...
        <Card title='点检负责人' extra={
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <a style={{ display: curitem.id ? "block" : "none" }} onClick={() => {
              this.setState({
                visible: true,
                iftype: {
                  name: `修改设备${curitem.equipmentName}的点检负责人`,
                  value: "edit"
                },
                curitem: curitem,
              })
            ...
            <Popconfirm
              ...
              title={"确认删除该点检负责人？"}
              onConfirm={() => {
                this.setNewState("equipmentdeleteByIds", { id: curitem.id }, () => {
                  let total = this.props.check.equipmentqueryList.total,
                    page = this.props.check.equipmentqueryList.pageNum;
                  if ((total - 1) % 9 == 0) {
                    page = page - 1
                  }
          ...
          <div style={{ height: fv ? "auto" : 0, overflow: "hidden", transition: "all 0.4s" }}>
            ...
            <div style={{ height: charge ? 0 : 700, overflow: "hidden", marginTop: charge ? 0 : 24, transition: "all 0.4s", display: "flex", justifyContent: "center", alignItems: "center" }}>
              <Empty description={
                <span>
                  请先选择 <a>点检负责人<Icon type="arrow-up" /></a>
                </span>
              } />
            </div>
          ...
          <Modal
            width={iftype.value == "edit" ? 600 : 1100}

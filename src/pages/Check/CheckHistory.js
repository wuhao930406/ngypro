...
this.props.ensureDidMount&&this.props.ensureDidMount()
  ...
  render() {
    let { fatequeryListOfEquipment } = this.props.check;
    ...
    const content = (part2) => (
      <div className={styles.limitdiv}>
        ...
        <p>点检状态：<span style={{ color: part2.pointCheckItemResultType == null ? "#999" : part2.pointCheckItemResultType == 0 ? "#0e6eb8" : "#ff2100" }}>{part2.pointCheckItemResultType == null ? "待执行" : part2.pointCheckItemResultType == 0 ? "执行正常" : "执行异常"}</span></p>
        <p>指标：<span>{part2.normalReference}</span></p>
    ...
    const renderPart2 = (part1, part2) => {
      if (part2) {
        return part2.map((item, i) => {
          return <Popover key={i} placement="bottomRight" content={content(item)} title={part1.pointCheckItem}>
            <div style={{ minWidth: 24, height: 24, lineHeight: "24px", backgroundColor: item.pointCheckItemResultType == null ? "#999" : item.pointCheckItemResultType == 0 ? "#0e6eb8" : "#ff2100", marginRight: 2, marginBottom: 2, textAlign: "center", cursor: "pointer", flex: 1, color: "#fff" }} >
              {
                part1.periodType == 0 ?
    ...
    return (<div>
      <div className={styles.setTab}>
        <Card title={this.props.match.params.name ? <span><a>{this.props.match.params.name}</a>的点检历史</span> : "点检历史记录"}>
          <div style={{ padding: "0 10px" }}>
            <Row gutter={24} style={{ padding: "10px 0", backgroundColor: "#f0f0f0" }}>
              <Col span={2} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span>点检时间</span>
                <Popover placement="bottomRight" content={contents()} trigger="click">
              ...
              <Col span={18}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-start" }}>
                  ...
                  待执行
                  <i style={{ width: 16, height: 16, margin: "0 4px 0px 8px", backgroundColor: "#0e6eb8", display: "block" }}></i>
                  执行正常

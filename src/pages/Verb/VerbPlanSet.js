import {
  Table, Input, InputNumber, Popconfirm, Form, Divider, Modal, Tree, Button, Row, Col, Icon, Select, Alert, Tag, message, Card, Empty,Tooltip,
  Transfer,
} from 'antd';
...
@connect(({ verb, loading }) => ({
  verb,
  submitting: loading.effects['verb/equipmentqueryList'],
  submittings: loading.effects['verb/equipmentqueryNoByUserId'],
  submittingc: loading.effects['verb/equipmentqueryByUserId'],
}))
class VerbPlanSet extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      postData: {
        ...
        "pageSize": 9,//int 页面条数*
        "chargeType": "1",//string 负责类型*(0维修,1保养)
        "userId": "",//Long 负责人id
  ...
  refreshData(ifs) {
    ...
    this.setNewState("equipmentsave", {
      "userId": charge,
      "chargeType": "1",
      "equipIds": haveId
    }, () => {
      ...
      this.setNewState("equipmentqueryByUserId", postDatac, () => {
        let res = this.props.verb.equipmentqueryByUserId.list;
        this.setState({
  ...
  render() {
    let { postData, postUrl, fv, fields, iftype, curitem, charge, selectedRowKeys, selectedRowKeyc, postDatas, postDatac } = this.state,
      { equipmentqueryList, userList, equipmentqueryNoByUserId, equipmentqueryByUserId, deviceTypequeryTreeList } = this.props.verb;
    const rowSelections = {
        ...
        <Card title='保养负责人列表' extra={
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <a style={{ display: curitem.id ? "block" : "none" }} onClick={() => {
              this.setState({
                visible: true,
                iftype: {
                  name: `修改设备${curitem.equipmentName}的保养负责人`,
                  value: "edit"
                },
                curitem: curitem,
              })
            ...
            <Popconfirm
              ...
              title={"确认删除该保养负责人？"}
              onConfirm={() => {
                this.setNewState("equipmentdeleteById", { id: curitem.id }, () => {
                  let total = this.props.verb.equipmentqueryList.total,
                    page = this.props.verb.equipmentqueryList.pageNum;
                  if ((total - 1) % 9 == 0) {
                    page = page - 1
                  }
              ...
              <a style={{ display: curitem.id ? "block" : "none", color: "#ff7000" }}>删除</a>
            </Popconfirm>
          ...
          <div style={{ height: fv ? "auto" : 0, overflow: "hidden", transition: "all 0.4s" }}>
            ...
            <div style={{ height: charge ? 0 : 700, overflow: "hidden", marginTop: charge ? 0 : 24, transition: "all 0.4s", display: "flex", justifyContent: "center", alignItems: "center" }}>
              <Empty description={
                <span>
                  请先选择 <a>保养负责人<Icon type="arrow-up" /></a>
                </span>
              } />
            </div>

    ...
    const expandedRowRender = () => {
      const columnss= [
        ...
        },
        {
          ...
          dataIndex: 'see',
          key: 'see',
          render:(text,record)=><Link to={`/devices/devicetzlists/deviceRecord/${this.state.curitem.id}/${record.dicKey}/${this.state.curitem.equipmentName?this.state.curitem.equipmentName.replace(/\s+/g,""):""}`}>
            查看履历</Link>
        },
       
      ];
      return <Table size="middle" scroll={{ x:1200,y:"59vh" }} columns={columnss} dataSource={resumetype} pagination={false} />;
    };

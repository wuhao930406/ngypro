...
@connect(({ chart, loading }) => ({
  chart,
  submitting: loading.effects['chart/queryJIA'],
}))
class ChartJia extends Component {
  constructor(props) {
    super(props);
    this.state = {
      postData: {
...
this.props.ensureDidMount&&this.props.ensureDidMount()
  ...
  getOption() {
    let allData = this.props.chart.queryJIA;
    let res = {}
  ...
  render() {
    const { postData } = this.state,
      { queryJIA, companyList, departmentList, shopList } = this.props.chart;

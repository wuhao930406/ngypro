...
@connect(({ chart, loading }) => ({
  chart,
  submitting: loading.effects['chart/queryOEE'],
}))
class ChartMoee extends Component {
  constructor(props) {
    super(props);
    this.state = {
      postData: {
...
this.props.ensureDidMount&&this.props.ensureDidMount()
  ...
  getOption() {
    let allData = this.props.chart.queryOEE;
    let res = {}
  ...
  render() {
    const { postData } = this.state,
      { queryOEE, companyList, departmentList, shopList } = this.props.chart;

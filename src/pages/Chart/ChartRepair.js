...
class ChartRepair extends Component {
  constructor(props) {
    super(props);
    this.state = {
      collspan: true,
      ...
      postDatas: {
        ...
        "endTime": "",//结束时间
        "departmentId": "",//部门id
        "repairStatus": ''//维修状态（未完成和已完成）   4已完成  其他表示未完成
      }
    };
  }
...
this.props.ensureDidMount&&this.props.ensureDidMount()
    ...
    this.setNewState('queryTimeChart', this.state.postDatas);
  ...
  getOptions(value) {
    let allData = this.props.chart.queryTimeChart;
    let res = {}
    ...
    let xDatas = allData[value].map((item, i) => {
      ...
      reportTime = allData[value].map((item, i) => {
        return (item.reportTime / 1000 / 60).toFixed(1)
      }),
      repairTime = allData[value].map((item, i) => {
        return (item.repairTime / 1000 / 60).toFixed(1)
      }),
      confirmTime = allData[value].map((item, i) => {
        return (item.confirmTime / 1000 / 60).toFixed(1)
      });

    res = {
      title: {
        text: `统计各条维修单的报修，维修，验证时间`,
        subtext: '',
        x: '0',
        textStyle: {
          fontSize: 16,
  ...
  render() {
    const { searchValue, expandedKeys, autoExpandParent, collspan, postData, key,postDatas  } = this.state,
      { companyDepartList, queryDiagram, deviceTypequeryTreeList, queryTimeChart } = this.props.chart;
              ...
              <div>
                ...
                <Card bordered={false} title={titlerender()} >
                    ...
                    key == "3" ?
                      <Row gutter={24}>
                        <Col xs={24} sm={24} md={7} lg={7} xl={7} xxl={7} style={{ marginBottom: 12 }}>
                          <Select placeholder="维修状态" allowClear style={{ width: "100%" }} value={postDatas.repairStatus} onChange={(value) => {
                            this.setState({
                              postDatas: { ...this.state.postDatas, repairStatus: value },
                            }, () => {
                              this.setNewState('queryTimeChart', { ...this.state.postDatas, repairStatus: value });
                            });
                          }}>
                        ...
                        <Col xs={24} sm={24} md={7} lg={7} xl={7} xxl={7} style={{ marginBottom: 12 }}>
                          <DatePicker.RangePicker style={{ width: "100%" }} value={postDatas.startTime ? [moment(postDatas.startTime), moment(postDatas.endTime)] : undefined}
                            onChange={val => {
                              this.setState(
                                {
                                  postDatas: { ...postDatas, startTime: val[0] ? moment(val[0]).format("YYYY-MM-DD") : null, endTime: val[1] ? moment(val[1]).format("YYYY-MM-DD") : null, pageIndex: 1 },
                                },
                                () => {
                                  this.setNewState('queryTimeChart', this.state.postDatas);
                                },
                        ...
                        <Col xs={24} sm={24} md={7} lg={7} xl={7} xxl={7} style={{ marginBottom: 12 }}>
                          <TreeSelect placeholder="所在部门" allowClear style={{ width: "100%" }} value={postDatas.departmentId} onChange={(value) => {
                            this.setState({
                              postDatas: { ...this.state.postDatas, departmentId: value },
                            }, () => {
                              this.setNewState('queryTimeChart', { ...this.state.postDatas, departmentId: value });
                            });
                          }}>
                  ...
                  <Tabs activeKey={key} onChange={(key) => {
                    ...
                    <TabPane tab="故障统计" key="3">
                      {
                        this.state.show && <Row gutter={24}>
                          <Col span={24} >
                            {
                              queryTimeChart.TIMECHART ?
                                <ReactEcharts style={{height:"62vh"}} option={this.getOptions("TIMECHART")}></ReactEcharts> :
                                <Empty style={{ margin: "12px 0px" }} description={
                                  <span>
                                    设备故障总数 - 暂无数据
                                  </span>
...
ChartRepair = Form.create('yangziges')(ChartRepair);

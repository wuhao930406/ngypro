...
@connect(({ sensor, loading }) => ({
  sensor,
  submittings: loading.effects['sensor/sensorTypequeryList'],
}))
class SensorType extends Component {
  ...
  setNewState(type, value, fn) {
    let { dispatch, sensor } = this.props;
    dispatch({
      type: 'sensor/' + type,
      payload: value,
    }).then(key => {
...
this.props.ensureDidMount&&this.props.ensureDidMount()
    this.setNewState('sensorTypequeryList', null, () => {
      this.setState({
  ...
  getConvert() {
    let { sensorTypequeryList } = this.props.sensor;
    let mostData = [
      {
        key: '0',
        title: '全部',
        children: sensorTypequeryList,
      },
    ];
    return mostData;
  }
  ...
  onSubmit() {
    ...
    this.setNewState('sensorTypesave', postData, () => {
      this.setNewState('sensorTypequeryList', null, () => {
        this.setState({
          gData: this.getConvert(),
        });
  ...
  onDelete() {
    ...
    confirm({
      ...
      onOk() {
        ...
        _it.setNewState('sensorTypedeleteById', postData, () => {
          _it.setNewState('sensorTypequeryList', null, () => {
            _it.setState({
  ...
  render() {
    ...
    { sensorTypequeryList } = this.props.sensor;
...
SensorType = Form.create('liziyuans')(SensorType);

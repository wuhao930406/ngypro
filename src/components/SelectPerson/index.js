  ...
  render() {
    ...
    let columns = [
      {
        ...
        render:(text,record) => <Tooltip placement="bottomLeft" title={text} >
          <a style={{display:"block",maxWidth:88,color:record.pkid==this.state.cur?"#f50":"#0e6eb8",
    ...
    return (
      <Modal
        ...
        visible={visible}
        title={type.name?type.show?"选择使用备件及验证人":'选择'+type.name:'选择人员'}
           ...
           <p className="fades">选择使用的备件</p>
            <Alert
              ...
              <div>
              {
                ...
                style={{marginBottom:8,cursor:"pointer"}}><span style={{color:"#0e6eb8"}}>{item.spareName}</span>:
                <span style={{color:item.spareNum?"#0e6eb8":"#ff4800"}}>
              ...
              type="info"/>
          <Table size="middle"  scroll={{x:"100%"}}
